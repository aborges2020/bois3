                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title float-sm-right">
                            <button type="button" name="create_record" id="create_record" class="btn btn-warning btn-sm"><i class="fas fa-plus-circle"></i> Add Shipping</button>
                        </h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
...
@section('js_scripts')
    <script type="text/javascript">
        $(document).ready(function(){
            var dt = $('#shippings_table').DataTable({
                // Order table
                order: [[ 0, "desc" ]],
                //stateSave: true,
                processing: false,
            ...
            $('#data_form').on('submit', function(event){
                ...
                if($('#action').val() == 'Add')
                {
                    action_url = "/admin/shipping-methods";
                }

                if($('#action').val() == 'Edit')
                {
                    action_url = "/admin/shipping-methods/update";
                }
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#shippings_table').DataTable().ajax.reload();
                        }
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_result').html('');
                $.ajax({
                    url :"/admin/shipping-methods/" + id + "/edit",
                    dataType:"json",
                    success:function(data)
                    {
            ...
            $('#ok_button').click(function(){
                $.ajax({
                    url:"/admin/shipping-methods/destroy/" + id,
                    beforeSend:function(){

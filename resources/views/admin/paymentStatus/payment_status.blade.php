                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title float-sm-right">
                            <button type="button" name="create_record" id="create_record" class="btn btn-warning btn-sm"><i class="fas fa-plus-circle"></i> Add Order Status</button>
                        </h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="paymentStatus_table" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>id</th>
                                    <th>Name</th>
...
@section('js_scripts')
    <script type="text/javascript">
        $(document).ready(function(){
            var dt = $('#paymentStatus_table').DataTable({
                // Order table
                order: [[ 0, "desc" ]],
                //stateSave: true,
            ...
            $('#data_form').on('submit', function(event){
                ...
                if($('#action').val() == 'Add')
                {
                    action_url = "/admin/payment-status";
                }

                if($('#action').val() == 'Edit')
                {
                    action_url = "/admin/payment-status/update";
                }
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#paymentStatus_table').DataTable().ajax.reload();
                        }
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_result').html('');
                $.ajax({
                    url :"/admin/payment-status/" + id + "/edit",
                    dataType:"json",
                    success:function(data)
                    {
            ...
            $('#ok_button').click(function(){
                $.ajax({
                    url:"/admin/payment-status/destroy/" + id,
                    beforeSend:function(){
